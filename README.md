...
> ## :memo: 마치며
백엔드와 프론트엔드, 데이터베이스를 학습하고 만든 첫 웹서비스 구현 프로젝트였습니다. 
JSP/Servlet을 처음 사용해보며 클라이언트와 서버가 데이터를 주고받는 방식을 배울수 있었습니다. 처음에는 모델, 뷰, 컨트롤러를 나눠서 작업하는 MVC 패턴에 익숙하지 않아 어려움이 있었지만 CRUD를 구현하며 작업이 이루어지는 로직을 이해할 수 있었습니다. 프로젝트 등록시 인원을 추가하는 과정에서 새로고침 없이 추가, 삭제를 해야 했기 때문에 ajax를 처음 사용해보았습니다. 동적으로 코드를 생성해주는 과정에서 오류가 났을때 디버깅에 어려움이 있었지만 error메세지를 활용하여 디버깅 처리를 보다 수월하게 하는 방법을 배웠습니다. 
